...
async function anchorToSolana(data, chainName, privateKeyBase58, customRpc = null) {
  ...
  if (dataSize > config.maxMemoSize) {
    throw new Error(`Data too large for Solana memo. Max: ${config.maxMemoSize} bytes, Got: ${dataSize} bytes.`);
  }
  ...
  if (balance === 0) throw new Error('Insufficient balance. Fund this address first.');
  ...
  return {
    proof: {
      ...
      explorer: config.explorer + signature
    }
  };
}

export { anchorToSolana };
